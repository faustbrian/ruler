...
final readonly class RuleCompiler
{
     ...
     */
    private function compileConstant(ConstantNode $node): BaseVariable
    {
     ...
     */
    private function compileNode(Node $node): Proposition|VariableOperand
    {
        return match (true) {
            $node instanceof ConstantNode => $this->compileConstant($node),
     ...
     * @return VariableOperand The resolved Variable or VariableProperty
     */
    private function compileName(NameNode $node): VariableOperand
    {
     ...
     * @return VariableOperand The resolved Variable or VariableProperty chain
     */
    private function compileGetAttr(GetAttrNode $node): VariableOperand
    {
    ...
    private function buildFieldPath(Node $object, Node $attribute): string
    {
        ...
        $this->throwUnsupportedNodeType(); // @codeCoverageIgnore
    }
     ...
     */
    private function compileBinary(BinaryNode $node): Proposition|VariableOperand
    {
     ...
     * @return Proposition|VariableOperand The compiled operator
     */
    private function compileUnary(UnaryNode $node): Proposition|VariableOperand
    {
        ...
        $operatorName = match ($operator) {
            'not' => 'not',
            '-' => 'negate',
            default => $this->throwUnsupportedUnaryOperator($operator), // @codeCoverageIgnore
        };
