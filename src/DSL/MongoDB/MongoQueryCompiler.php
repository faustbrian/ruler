...
final readonly class MongoQueryCompiler
{
     ...
     *
     * @param VariableOperand       $field Field variable to test
     ...
     *
     * @param VariableOperand        $field Field variable containing a date value to test
     ...
     *
     * @param  VariableOperand $field       Field variable to test for existence
     ...
     */
    private function compileExists(VariableOperand $field, bool $shouldExist): Proposition
    {
        $nullCheck = new EqualTo($field, new Variable());
     ...
     *
     * @param  VariableOperand $field   Field variable to match against
     ...
     */
    private function compileRegex(VariableOperand $field, string $pattern, string $options): Proposition
    {
     ...
     *
     * @param VariableOperand $field      Field variable containing string to measure
     ...
     */
    private function compileStringLength(VariableOperand $field, mixed $comparison): Proposition
    {
     ...
     *
     * @param VariableOperand $field Field variable to type-check
     ...
     */
    private function compileType(VariableOperand $field, string $type): Proposition
    {
        return match ($type) {
     ...
     *
     * @param VariableOperand $field      Field variable containing array to count
     * @param mixed           $comparison Either a number for exact match or comparison operator object
     *
     * @throws InvalidArgumentException If comparison is neither number nor valid operator object
     *
     ...
     */
    private function compileArrayCount(VariableOperand $field, mixed $comparison): Proposition
    {
     ...
     *
     * @param  VariableOperand $field         Field variable to test for emptiness
     ...
     */
    private function compileEmpty(VariableOperand $field, bool $shouldBeEmpty): Proposition
    {
    ...
    private function compileFieldCondition(string $field, mixed $value): Proposition
    {
        ...
        foreach ($value as $operator => $operand) {
            ...
            $conditions[] = match ($operator) {
                ...
                '$strLength' => $this->compileStringLength($fieldVar, $operand),
                // Date operators
    ...
    {
        $props = array_map($this->compileQuery(...), $conditions);
