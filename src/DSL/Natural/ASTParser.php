 ...
 *
 * @internal This is an internal implementation class. Use NaturalLanguageParser facade instead.
 */
final class ASTParser
{
     ...
     */
    private function parseCondition(string $text): array
    {
        // Try patterns in order of specificity
        // Note: between and list membership already tried in parseExpression
        if (($node = $this->tryParseStringOperation($text)) !== null && ($node = $this->tryParseStringOperation($text)) !== []) {
            return $node;
        }

        if (($node = $this->tryParseComparison($text)) !== null && ($node = $this->tryParseComparison($text)) !== []) {
            return $node;
        }
     ...
     */
    private function tryParseBetween(string $text): ?array
    {
        ...
        if (preg_match($pattern, $text, $matches)) {
            return [
                'type' => 'between',
                'field' => $matches[1],
                'min' => $this->parseValue(mb_trim($matches[2])),
                'max' => $this->parseValue(mb_trim($matches[3])),
            ];
        }
     ...
     * @return null|array<string, mixed> AST node if matched, null otherwise
     */
    private function tryParseListMembership(string $text): ?array
    {
        ...
        if (preg_match($pattern, $text, $matches)) {
            return [
                ...
                'field' => $matches[1],
                'values' => [
                    $this->parseValue(mb_trim($matches[2])),
                    $this->parseValue(mb_trim($matches[3])),
                ],
                'negated' => false,
            ];
        }
        ...
        if (preg_match($pattern, $text, $matches)) {
            $values = array_map(
                fn (mixed $v): mixed => $this->parseValue(mb_trim($v)),
                explode(',', $matches[3]),
            );
     ...
     * @return null|array<string, mixed> AST node if matched, null otherwise
     */
    private function tryParseStringOperation(string $text): ?array
    {
        $operations = [
            'contains' => 'contains',
            'includes' => 'contains',
     ...
     * @return null|array<string, mixed> AST node if matched, null otherwise
     */
    private function tryParseComparison(string $text): ?array
    {
    ...
    private function parseExpression(string $text): array
    {
        ...
        if (($node = $this->tryParseBetween($text)) !== null && ($node = $this->tryParseBetween($text)) !== []) {
            return $node;
        }

        if (($node = $this->tryParseListMembership($text)) !== null && ($node = $this->tryParseListMembership($text)) !== []) {
            return $node;
        }
        ...
        if (count($orParts) > 1) {
            return [
                'type' => 'logical',
                'operator' => 'or',
                'conditions' => array_map($this->parseExpression(...), $orParts),
            ];
        }
        ...
        if (count($andParts) > 1) {
            return [
                'type' => 'logical',
                'operator' => 'and',
                'conditions' => array_map($this->parseExpression(...), $andParts),
            ];
        }
