...
/**
 * Minimum value extraction operator for finding the lowest value in a collection.
 *
 * Extracts and returns the minimum value from a collection or set. The operand must
 * resolve to a Value containing a set of comparable values. The operation delegates
 * to the set's min() method to identify the lowest value.
 *
 * ```php
 * $min = new Min();
 * $min->addOperand(new Variable('prices')); // prices is a collection
 * $lowest = $min->prepareValue($context); // returns lowest price
 * ```
 *
...
{
    /**
     * Prepares the minimum value from the operand's collection.
     *
     * Evaluates the operand within the provided context to resolve its value, extracts
     * the set from that value, and returns the minimum value found in the set.
     *
     * @param  Context $context Evaluation context containing variable values and facts
     *                          required for operand resolution
     * @return Value   Value object containing the minimum value from the collection
     */
