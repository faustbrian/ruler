...
final class Intersect extends VariableOperator implements VariableOperand
{
     ...
     * @return Value   The resulting set containing only elements present in all operand sets
     */
    public function prepareValue(Context $context): Value
    {
        ...
        foreach ($this->getOperands() as $operand) {
            if (!$intersect instanceof Set) {
                ...
                $intersect = $intersect->intersect($set->asValue());
            }
        }

        return $intersect?->asValue() ?? new Value([]);
    }
