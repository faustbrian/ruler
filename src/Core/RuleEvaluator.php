        ...
        if (array_key_exists('combinator', $rule)) {
            ...
            if ($rule['combinator'] === 'not') {
                ...
                if (count($rule['value']) !== 1) {
                    throw RuleEvaluatorException::invalidNotRule();
                }
            ...
            if (!in_array($rule['combinator'], ['and', 'or', 'xor'], true)) {
                throw RuleEvaluatorException::invalidCombinator($rule['combinator']);
            }
        ...
        if (array_key_exists('operator', $rule)) {
            ...
            $builder = str_contains($fieldString, '.')
                ? array_reduce(
                    explode('.', $fieldString),
                    /**
                     * @param  RuleBuilder|BuilderVariable|VariableProperty $builder
                     * @return BuilderVariable|VariableProperty
                     */
                    static function (mixed $builder, string $segment): mixed {
