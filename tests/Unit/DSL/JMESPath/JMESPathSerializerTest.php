...
describe('JMESPathSerializer', function (): void {
    describe('Happy Paths', function (): void {
        ...
        test('serialize nested property access', function (): void {
            ...
            $rule = $parser->parse('user.age >= `18`', RuleIds::fromString('test-rule'));
            $result = $serializer->serialize($rule);
        ...
        test('serialize with contains function', function (): void {
            ...
            $rule = $parser->parse('contains(tags, `"php"`)', RuleIds::fromString('test-rule'));
            $result = $serializer->serialize($rule);
        ...
        test('serialize with starts_with function', function (): void {
            ...
            $rule = $parser->parse('starts_with(email, `"admin"`)', RuleIds::fromString('test-rule'));
            $result = $serializer->serialize($rule);
        ...
        test('serialize complex nested expression', function (): void {
            ...
            $rule = $parser->parse('(age >= `18` && country == `"US"`) || age >= `21`', RuleIds::fromString('test-rule'));
            $result = $serializer->serialize($rule);
        ...
        test('serialize with array filter', function (): void {
            ...
            $rule = $parser->parse('users[?age > `18`]', RuleIds::fromString('test-rule'));
            $result = $serializer->serialize($rule);
        ...
        test('serialize with projection', function (): void {
            ...
            $rule = $parser->parse('users[*].age', RuleIds::fromString('test-rule'));
            $result = $serializer->serialize($rule);
        ...
        test('serialize with pipe expression', function (): void {
            ...
            $rule = $parser->parse('users | length(@) > `5`', RuleIds::fromString('test-rule'));
            $result = $serializer->serialize($rule);
