...
describe('Variable', function (): void {
    describe('Happy Paths', function (): void {
        ...
        test('offsetSet sets property default value', function (): void {
            $rb = new RuleBuilder();
            $var = new Variable($rb, 'user');

            $var['email'] = 'test@example.com';

            expect($var['email'])->toBeInstanceOf(VariableProperty::class);
            expect($var['email']->getValue())->toBe('test@example.com');

            $context = new Context([]);
            expect($var['email']->prepareValue($context)->getValue())->toBe('test@example.com');
        });

        test('notSameAs creates strict inequality comparison', function (): void {
            $rb = new RuleBuilder();
            $varA = new Variable($rb, 'a');
            $varB = new Variable($rb, 'b');
            ...
            expect($varA->notSameAs($varB))->toBeInstanceOf(NotSameAs::class);
            expect($varA->notSameAs($varB)->evaluate($context))->toBeTrue();
            expect($varA->notSameAs(1)->evaluate($context))->toBeFalse();
        });

        test('sameAs creates strict equality comparison', function (): void {
            $rb = new RuleBuilder();
            $varA = new Variable($rb, 'a');

            $context = new Context(['a' => 1]);

            expect($varA->sameAs(1))->toBeInstanceOf(SameAs::class);
            expect($varA->sameAs(1)->evaluate($context))->toBeTrue();
            expect($varA->sameAs('1')->evaluate($context))->toBeFalse();
        });

        test('min extracts minimum value from collection', function (): void {
            $rb = new RuleBuilder();
            $var = new Variable($rb, 'numbers');
            ...
            $minVar = $var->min();
            expect($minVar)->toBeInstanceOf(Variable::class);
            expect($minVar->getValue())->toBeInstanceOf(Min::class);
            expect($minVar->prepareValue($context)->getValue())->toBe(1);
        });

        test('max extracts maximum value from collection', function (): void {
            $rb = new RuleBuilder();
            $var = new Variable($rb, 'numbers');
            ...
            $maxVar = $var->max();
            expect($maxVar)->toBeInstanceOf(Variable::class);
            expect($maxVar->getValue())->toBeInstanceOf(Max::class);
            expect($maxVar->prepareValue($context)->getValue())->toBe(9);
        });

        test('containsSubset checks if set contains all elements of another set', function (): void {
            $rb = new RuleBuilder();
            $varA = new Variable($rb, 'a');
            $varB = new Variable($rb, 'b');
            ...
            expect($varA->containsSubset($varB))->toBeInstanceOf(ContainsSubset::class);
            expect($varA->containsSubset($varB)->evaluate($context))->toBeTrue();
            expect($varA->containsSubset([2, 3, 6])->evaluate($context))->toBeFalse();
        });

        test('doesNotContainSubset checks if set does not contain all elements of another set', function (): void {
            $rb = new RuleBuilder();
            $varA = new Variable($rb, 'a');
            $varB = new Variable($rb, 'b');
            ...
            expect($varA->doesNotContainSubset($varB))->toBeInstanceOf(DoesNotContainSubset::class);
            expect($varA->doesNotContainSubset($varB)->evaluate($context))->toBeTrue();
            expect($varA->doesNotContainSubset([2, 3])->evaluate($context))->toBeFalse();
        });

        test('__call wraps VariableOperator in new Variable for chaining', function (): void {
            $rb = new RuleBuilder();
            $rb->registerOperatorNamespace('Cline\\Ruler\\Operators\\Mathematical');

            $var = new Variable($rb, 'value');
            $context = new Context(['value' => 5]);
